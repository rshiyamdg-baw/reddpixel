...
const WorksPhaseUI: React.FC = () => {
  ...
  const handleClose = (e: React.MouseEvent) => {
    ...
    setTimeout(() => setIsClosing(false), 700); 
  }
  ...
  useGSAP(() => {
    ...
    tl.current = gsap.timeline({ paused: true })
      ...
      .fromTo('.anim-node', { scale: 0, rotate: 0 }, { scale: 1, rotate: 45, duration: 0.6, stagger: 0.1, ease: 'back.out(2)' })
      .fromTo('.anim-line', { scaleY: 0 }, { scaleY: 1, duration: 0.4, stagger: 0.1, ease: 'power2.inOut' }, "-=0.1")
      .fromTo(cards, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, stagger: 0.1, ease: 'power2.out' }, "-=0.2")
      .fromTo('.anim-content', { opacity: 0, y: 15 }, { opacity: 1, y: 0, duration: 0.4, stagger: 0.05 }, "-=0.3")
  }, { scope: containerRef })

  useGSAP(() => {
    if (expandedId !== null && !isClosing) {
      gsap.to(`#node-${expandedId}`, { scale: 1.3, rotate: 90, duration: 0.4, ease: 'power2.out' });
      gsap.to(`#line-${expandedId}`, { scaleX: 1.5, duration: 0.4, ease: 'power2.out' });

      PROJECTS.forEach(p => {
         if(p.id !== expandedId) {
            gsap.to(`#node-${p.id}`, { opacity: 0.2, rotate: 45, scale: 0.9, duration: 0.4 });
            gsap.to(`#line-${p.id}`, { opacity: 0.2, scaleX: 1, duration: 0.4 });
         } else {
            gsap.to(`#node-${p.id}`, { opacity: 1, duration: 0.4 });
            gsap.to(`#line-${p.id}`, { opacity: 0.7, duration: 0.4 });
         }
      });
    } else if (expandedId === null) {
      gsap.to('.anim-node', { scale: 1, rotate: 45, opacity: 1, duration: 0.4, ease: 'power2.out' });
      gsap.to('.anim-line', { opacity: 0.7, scaleX: 1, duration: 0.4 });
    }
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-40 flex flex-col items-center justify-center p-4 sm:p-10 pointer-events-none pb-28 sm:pb-32">
      
      {/* THE FIX: Taller Master Container (h-[70vh] max-h-[650px]) */}
      <div className="flex w-full max-w-6xl h-[70vh] max-h-[650px] gap-3 md:gap-5 pointer-events-auto z-10">
        {PROJECTS.map((project) => {
          ...
          return (
            <div 
              ...
              className={`project-window group relative flex overflow-hidden border transition-all duration-700 ease-[cubic-bezier(0.87,0,0.13,1)] bg-[#050505] md:bg-[#050505]/95 ${project.theme.border} hover:border-opacity-100
            ...
            >
              ...
              <div className={`absolute inset-0 flex items-center justify-center transition-opacity ${isSiblingExpanded ? 'opacity-100 duration-500 delay-300' : 'opacity-0 duration-300 pointer-events-none'}`}>
              ...
              {/* MAIN COMPACT VIEW */}
              <div className={`absolute inset-0 flex flex-col items-center transition-opacity ${expandedId === null ? 'opacity-100 duration-700 delay-300' : 'opacity-0 duration-300 pointer-events-none'}`}>
                 ...
                 <div className="relative z-10 flex flex-col items-center text-center mt-auto mb-10 px-4">
                    <span className={`anim-content mb-2 text-[10px] uppercase tracking-[0.4em] opacity-80 ${project.theme.accent}`}>{project.category}</span>
                    <h3 className={`anim-content text-lg md:text-2xl font-light tracking-[0.2em] ${project.theme.text}`}>{project.title}</h3>
                 </div>
              </div>

              {/* EXPANDED VIEW */}
              <div className={`absolute inset-0 flex flex-col md:flex-row transition-opacity ${isExpanded ? 'opacity-100 duration-500 delay-400' : 'opacity-0 duration-300 pointer-events-none'}`}>
                ...
                {/* THE FIX: Image claims 55% of the width on Desktop, and 45% of the height on Mobile! */}
                ...
                {/* THE FIX: Text box shrinks to 45% of width, forcing shorter lines and better wrapping! */}
                <div className="relative w-full h-[55%] md:h-full md:w-[45%] flex flex-col justify-center p-6 md:p-12 bg-[#050505] md:bg-transparent z-10 overflow-y-auto hide-scrollbar">
                  
                  <div className="absolute top-6 md:top-10 left-1/2 -translate-x-1/2 flex items-center justify-center gap-3 opacity-40">
                  ...
                  <h2 className={`mb-4 md:mb-6 text-2xl md:text-4xl lg:text-5xl font-light tracking-[0.15em] ${project.theme.text}`}>{project.title}</h2>
                  <p className={`leading-relaxed font-light text-[11px] md:text-sm opacity-80 max-w-[90%] ${project.theme.text}`}>{project.description}</p>
                  
                  {project.link ? (
                     <a href={project.link} target="_blank" rel="noreferrer" className={`group/btn relative w-fit mt-6 md:mt-8 px-6 py-3 overflow-hidden border transition-colors ${project.theme.borderBright} hover:border-white`}>
                        <div className={`absolute inset-0 w-0 transition-all duration-500 ease-out group-hover/btn:w-full ${project.theme.solidBg} opacity-20`} />
                        <span className={`relative text-[10px] tracking-[0.2em] font-bold transition-colors group-hover/btn:text-white ${project.theme.accent}`}>INITIATE SEQUENCE</span>
                     </a>
                  ) : (
                     <div className={`w-fit mt-6 md:mt-8 flex items-center gap-3 border px-5 py-3 bg-black/40 ${project.theme.border}`}>
                        <div className={`w-2 h-2 rounded-full animate-pulse ${project.theme.solidBg} ${project.theme.shadow}`} />
                        <span className={`text-[10px] tracking-widest ${project.theme.text} opacity-80`}>PHYSICAL ARTIFACT - CLASSIFIED</span>
                     </div>
                  )}
                </div>
              </div>
      ...
      <div className="w-full max-w-6xl flex gap-3 md:gap-5 mt-2 px-4 md:px-0 z-0">
        {PROJECTS.map((project) => {
          const isExpanded = expandedId === project.id
          const isSiblingExpanded = expandedId !== null && expandedId !== project.id
          ...
          return (
             <div 
               key={`rig-${project.id}`} 
               className={`flex flex-col items-center transition-all duration-700 ease-[cubic-bezier(0.87,0,0.13,1)]
